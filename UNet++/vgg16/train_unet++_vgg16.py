...
# Suppress warnings from CarbonTracker and external APIs
...
def UNet_VGG16_sincong(loss="BCE", alpha=1, gamma=2, num_epochs=10, batch_size=4, shuffle_train=True, shuffle_test=False, model_name="UNetPlusPlus_VGG16"):
    clean_up()  # Clear memory and previous session variables
    ...
    class CorrosionDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            mask = (np.array(mask) > 0).astype(np.float32)  # Convert to binary mask
    ...
    # Initialize UNet++ model with VGG16 encoder backbone
    ...
    show_model_summary(model)  # Display model structure and parameter summary

    # Define loss function: Binary Cross Entropy or Focal Loss
    if loss == "BCE":
    ...
    # Run training and evaluation with carbon tracking
